...
%plot elements
for i = 1 : NoE
    H = plot( [NL(EL(i,1),1), NL(EL(i,2),1)], [NL(EL(i,1),2),NL(EL(i,2),2)], "LineWidth", 3, "Color", "w");
    H.Color(4) = 0.2; % alpha = 0.2

end

% plot nodes
for i = 1 : NoN
    plot(NL(i,1), NL(i,2), 'o', 'MarkerSize', 20, 'MarkerFaceColor', 'r', 'DisplayName', sprintf('Node %d', i));
end
...
colorMap = jet; % another one is "winter"
colormap(colormap);
...
for i = 1 : NoE
    ...
    sigma = E * (sqrt((ENL(EL(i,1),9)-ENL(EL(i,2),9))^2 + (ENL(EL(i,1),10)-ENL(EL(i,2),10))^2)...
        / (sqrt((ENL(EL(i,1),1)-ENL(EL(i,2),1))^2 + (ENL(EL(i,1),2)-ENL(EL(i,2),2))^2))); % sigma = E * epsilon | epsilon = delta(u)/u

    col = [sigma sigma]; % no color (stress) variation within the element
    figure(1)
...
if isequal(Element_flag,"on")
    ...
    for i = 1 : NoE
        ...
        text((ENL(EL(i,1),1)+mag*ENL(EL(i,1),9)+ENL(EL(i,2),1)+mag*ENL(EL(i,2),9))/2,(ENL(EL(i,1),2)+mag*ENL(EL(i,1),10)+ENL(EL(i,2),2)+mag*ENL(EL(i,2),10))/2,...
            num2str(i),"Color","w","FontSize",16,"HorizontalAlignment","center")
...
for i = 1 : NoN
    ...
    plot(ENL(i,1)+mag*ENL(i,9),ENL(i,2)+mag*ENL(i,10),"o","MarkerSize",16,"MarkerEdgeColor","r","MarkerFaceColor",[0,0.7,0.9]); % percent RGB
